...
class Droplet:
	...
	def importFromURI(self, uri, absl=False):
		print("entrando en funcion: importFromURI")
		if not absl:
	...
	def prepareBitmap(self, w, h):
		print("entrando en funcion: prepareBitmap")
	...
	def reshaperect(self, obj, allocation, radius=0):
		print("entrando en funcion: reshaperect")
	...
	def reshapecircle(self, obj, allocation):
		print("entrando en funcion: reshapecircle")
	...
	def reshapemask(self, obj, allocation, mask):
		print("entrando en funcion: reshapemask")
		bitmap = self.prepareBitmap(allocation.width, allocation.height)
	...
	def transparent_window (self, widget):
		print("entrando en funcion: transparent_window")
	...
	def transparent_expose (self, widget, event):
		print("entrando en funcion: transparent_expose")
		try:
	...
	def dict_key_exists(self, dict, key):
		print("entrando en funcion: dict_key_exists")
		return key in dict
	...
	def on_configure(self, w, e):
		print("entrando en funcion: on_configure")
	...
	def on_focus_out(self, w=None, e=None):
		print("entrando en funcion: on_focus_out")
	...
	def recieve(self, msg):
		print("entrando en funcion: recieve")
		if msg != 'null' and self.manifest.origin == 'local':
			...
			try:
				...
				print("Ocurrio un error inesperado. Function: recieve")
				print(e)
		else:
			print("Mensaje es NULL o el origen no es local")
			print("mensaje: "+str(msg))
			print("origin: "+str(self.manifest.origin))
				
	##
	# Convers a gtk events and DND gtk events to JSON to enable the connection from JavaScript
	def event_to_json(self, w, *args):
		print("entrando en funcion: event_to_json")
		try:
			dict = {}
			#on gtk event
			...
			print("Ocurrio un error inesperado. Function: event_to_json")
			print(e)
	
	
	def attrs_to_dict(self, obj):
		print("entrando en funcion: attrs_to_dict")
		try:
			attribs = dir(obj)
			result = {}
			for v in attribs:
			...
			print("Ocurrio un error inesperado. Function: attrs_to_dict")
			print(e)
	
	
	def send(self, msg):
		print("Entrando a funcion send")
	...
	def droplet_move_disable(self, browser=None, manifest=None):
		print("entrando en funcion: droplet_move_disable")
		if browser is None: browser = self.browser
		if manifest is None: manifest = self.manifest
	...
	def droplet_move(self, x, y):
		print("entrando en funcion: droplet_move")
		self.window.move(int(x), int(y))
	
	def droplet_deactivate(self, w=None, e=None):
		print("entrando en funcion: droplet_deactivate")
		self.on_focus_out()
		gtk.main_quit()
		raise SystemExit	
	
	def set_to_transparent(self):
		print("entrando en funcion: set_to_transparent")
	...
	def toggleProperty(self, w, e):
		print("entrando en funcion: toggleProperty")
		label = w.get_label()
	...
	def widgetContextMenu(self):
		print("entrando en funcion: widgetContextMenu")
		...
		def detachFn(*args):
			print("entrando en funcion: detachFn")
			print(args)
	...
	def prepare_widget(self, manifest, module, path):
		print("entrando en funcion: prepare_widget")
		window = gtk.Window()
		self.window = window
		...
		if manifest.origin == 'local': 
			...
			if module != None:
				print("entrando en funcion: on_title_change_wrapper")
		...
		def redirectNavigLocal (web_view, frame, request, navigation_action, policy_decision):
			print("entrando en funcion: redirectNavigLocal")
		...
		def redirectNavigRemote (web_view, frame, request, navigation_action, policy_decision):
			print("entrando en funcion: redirectNavigRemote")
			reason = re.match('^WEBKIT_WEB_NAVIGATION_REASON_(?P<reason>[A-Z0-9_]*)$', navigation_action.get_reason().value_name).group('reason')
			uri = request.get_uri()
		...
		def banRemoteRequests (web_view, frame, web_resource, request, response):
			print("entrando en funcion: banRemoteRequests")
		...
		def banOtherMime (web_view, frame, request, mimetype, policy_decision):
			print("entrando en funcion: banOtherMime")
		...
		print("entrando en funcion: on_load_wrapper")
	...
	def load_widget(self, browser, origin, source):
		print("entrando en funcion: load_widget")
		if origin != 'hosted':
	...
	def init_widget(self, path, custom_manifest = None):
		print("entrando en funcion: init_widget")
	...
	def __init__(self, path, custom_manifest = None):
		print("entrando en funcion: __init__")
